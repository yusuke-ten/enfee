import React from 'react';
import styled, { css } from 'styled-components';
import { Size, Color } from '../../../const';

type Type = 'h1' | 'h2' | 'h3';
type Color = 'base' | 'dark' | 'light' | 'less' | 'primary';

interface Props {
  type: Type;
  children: React.ReactNode;
  color?: Color;
}

const Heading: React.FC<Props> = ({ type, children, color = 'base' }) => {
  switch (type) {
    case 'h1':
      return <StyledH1 color={color}>{children}</StyledH1>;
    case 'h2':
      return <StyledH2 color={color}>{children}</StyledH2>;
    case 'h3':
      return <StyledH3 color={color}>{children}</StyledH3>;
    default:
      return null;
  }
};

const pallete = {
  base: Color.FONT.BASE,
  dark: Color.FONT.DARK,
  less: Color.FONT.LESS,
  light: Color.FONT.LIGHT,
  primary: Color.THEME.PRIMARY,
};

const getcolor = (color: Color) => pallete[color];

const commonStyle = css`
  font-weight: ${Size.FONT_WEIGHT.BOLD};
  letter-spacing: 2px;
`;
const StyledH1 = styled.h1<{ color: Color }>`
  color: ${({ color }) => getcolor(color)};
  font-size: ${Size.FONT.XXLARGE}px;
  ${commonStyle}
`;
const StyledH2 = styled.h2<{ color: Color }>`
  color: ${({ color }) => getcolor(color)};
  font-size: ${Size.FONT.XLARGE}px;
  ${commonStyle}
`;
const StyledH3 = styled.h3<{ color: Color }>`
  color: ${({ color }) => getcolor(color)};
  font-size: ${Size.FONT.LARGE}px;
  ${commonStyle}
`;

export default Heading;
